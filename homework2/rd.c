...
rd_handler_on(pid_t process)
{
    ...
    if (rd_type == 1 || rd_type == 2) {
        ...
        if (access(rd_target, W_OK) == 0) {
            if (noclobber == 1) {
                printf("YuqiShell: error: unable to redirect because of noclobber constraints: cannot overwrite\n");
                return;
            }
        }
    }
    ...
    if (rd_type == 3 || rd_type == 4) {
        ...
        if (access(rd_target, W_OK) == -1) {
            if (noclobber == 1) {
                if (errno == ENOENT) {
                    printf("YuqiShell: error: unable to redirect because of noclobber constraints: cannot create new file for appending\n");
                    return;
                }
            }
        }
...
check_rd(int argc, char** argv)
{
    ...
    for (i = 1; i <= argc - 2; i++) {
        if ( strncmp(argv[i], "<", 1) == 0 ||
                strncmp(argv[i], ">", 1) == 0 ) {
            if (argc - i != 2) {
                printf("YuqiShell: rd: syntax error!\n");
                return SYNTAX_ERROR;
            }
...
{
    if (argc > 1) {
        printf("YuqiShell: noclobber: syntax error!\n");
        return;
    }
    if (noclobber == 0) {
        noclobber = 1;
        printf("YuqiShell: noclobber is ON now!\n");
    }
    else if (noclobber == 1) {
        noclobber = 0;
        printf("YuqiShell: noclobber is OFF now!\n");
    }
    return;
}

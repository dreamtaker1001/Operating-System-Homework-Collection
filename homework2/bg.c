...
bg_show()
{
  printf("YuqiShell: [current bg jobs]:\n");
  if (bg_top == -1) {
...
{
  if (bg_top == MAXBG-1) {
    printf("YuqiShell: error: too many background jobs!\n");
    return 1;
  }
  bg_top++;
...
{
  if (bg_top == -1) {
    printf("YuqiShell: error: no background job to remove!\n");
    return -1;
  }
  int i;
...
{
    if (argc > 2) {
        printf("YuqiShell: fg: too many arguments!\n");
        return SYNTAX_ERROR;
    }
    else if (argc == 2) {
        if (atoi(argv[1]) > bg_top + 1) {
            printf("YuqiShell: fg: no such job!\n");
            return OTHER_ERROR;
        } 
        else {
            int i = atoi(argv[1]) - 1;
        ...
        if (bg_top == -1) {
            printf("YuqiShell: no background jobs!\n");
            return OTHER_ERROR;
        }
        else {
